 ...
 */
extern size_t hashmap_idx_sizeof(uint32_t count);
extern void cuckoo_index_reset(struct hashmap_idx_s *queue);
extern struct hashmap_idx_s *cuckoo_index_map(void *m, size_t count);

static inline int
cuckoo_free_index(struct hashmap_idx_s *queue,
...
static inline int
cuckoo_alloc_index(struct hashmap_idx_s *queue,
                   uint32_t *index,
                   unsigned n)
{
    if (n > queue->num)
